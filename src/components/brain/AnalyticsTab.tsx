...
const mockUsageTrend = [
  { date: '01-01', usage: 120, success: 115 },
  { date: '01-02', usage: 132, success: 128 },
  { date: '01-03', usage: 101, success: 98 },
  { date: '01-04', usage: 134, success: 130 },
  { date: '01-05', usage: 190, success: 185 },
  { date: '01-06', usage: 230, success: 225 },
  { date: '01-07', usage: 210, success: 205 },
];
...
const mockChannelDist = [
  { name: '短信', value: 35, count: 4200 },
  { name: 'APP推送', value: 28, count: 3360 },
  { name: '企业微信', value: 18, count: 2160 },
  { name: '外呼', value: 12, count: 1440 },
  { name: '邮件', value: 7, count: 840 },
];

const mockSegmentCoverage = [
...
export function AnalyticsTab({ atomId, atomName }: { atomId?: string; atomName?: string }) {
  const [usageTrendData, setUsageTrendData] = useState(mockUsageTrend);

  useEffect(() => {
    api.get('/performance/trend', { params: { timeRange: 'week' } })
      .then(res => {
        if (res.data?.data?.length) {
          setUsageTrendData(res.data.data);
        }
      })
      .catch(() => {});
  }, [atomId]);
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500">近7天调用量与成功量对比</p>
            </div>
          </div>
        ...
        <div className="h-72">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={usageTrendData}>
              ...
              <Legend />
              <Line type="monotone" dataKey="usage" stroke="#6366f1" strokeWidth={2} dot={false} name="调用量" />
              <Line type="monotone" dataKey="success" stroke="#10b981" strokeWidth={2} dot={false} name="成功量" />
            </LineChart>
          </ResponsiveContainer>
        </div>
      </Card>
      ...
      <Card className="p-6">
        ...
        <div className="h-72">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={mockSuccessRate} layout="vertical">
              <CartesianGrid strokeDasharray="3 3" stroke="#e2e8f0" />
              <XAxis type="number" domain={[0, 100]} stroke="#64748b" fontSize={12} unit="%" />
              <YAxis dataKey="name" type="category" stroke="#64748b" fontSize={12} width={80} />
              <Tooltip formatter={(value: number) => [`${value}%`, '成功率']} contentStyle={{ borderRadius: '8px', border: 'none', boxShadow: '0 4px 6px -1px rgb(0 0 0 / 0.1)' }} />
              <Legend />
      ...
      <Card className="p-6">
        ...
        <div className="grid grid-cols-3 gap-6">
          <div className="col-span-1 h-64">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie data={mockChannelDist} cx="50%" cy="50%" innerRadius={60} outerRadius={80} paddingAngle={2} dataKey="value">
                  {mockChannelDist.map((_, index) => (
                    <Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} />
                  ))}
                </Pie>
                <Tooltip formatter={(value: number, name: string, props: any) => [`${value}% (${props.payload.count}次)`, name]} contentStyle={{ borderRadius: '8px', border: 'none', boxShadow: '0 4px 6px -1px rgb(0 0 0 / 0.1)' }} />
              </PieChart>
            </ResponsiveContainer>
          </div>
          <div className="col-span-2">
            <div className="space-y-3">
              {mockChannelDist.map((item, index) => (
                <div key={item.name} className="flex items-center gap-3">
                  <div className="w-4 h-4 rounded-full" style={{ backgroundColor: COLORS[index % COLORS.length] }} />
                  <span className="text-sm font-medium text-slate-700 w-20">{item.name}</span>
                  <div className="flex-1 h-2 bg-slate-100 rounded-full overflow-hidden">
                    <div className="h-full rounded-full transition-all" style={{ width: `${item.value}%`, backgroundColor: COLORS[index % COLORS.length] }} />
                  </div>
                  <span className="text-sm font-bold text-slate-900 w-12 text-right">{item.value}%</span>
                  <span className="text-xs text-slate-500 w-20 text-right">{item.count.toLocaleString()}次</span>
                </div>
              ))}
            </div>
          </div>
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-4 text-sm">
                  <span className="text-slate-500">
                    已覆盖 <span className="font-bold text-slate-900">{item.covered.toLocaleString()}</span> / {item.total.toLocaleString()}
                  </span>
                  <span className="font-bold text-indigo-600 w-12 text-right">{item.coverage}%</span>
                </div>
              </div>
              <div className="h-3 bg-slate-100 rounded-full overflow-hidden">
                <div className="h-full bg-gradient-to-r from-indigo-500 to-violet-500 rounded-full transition-all" style={{ width: `${item.coverage}%` }} />
              </div>
            </div>
          ))}
        </div>
